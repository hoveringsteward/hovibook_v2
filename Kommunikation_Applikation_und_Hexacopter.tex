...
Eclipse ist eine freie Entwicklungsumgebung für Programme und wurde von der Eclipse Foundation entwickelt. Das Programm kann verwendet werden, um in mehreren Programmiersprachen Software zu entwicklen. Besonders gut geeignet ist Eclipse für die Entwicklung von Java-Programmen, da es sowohl Auto-Completion ermöglicht als auch Syntax-Hervorhebung.
      ...
      \subsubsection{Kommunikationsschema}
Für die Kommunikation zwischen dem Hexacopter und dem Server wurde folgendes Schema festgelegt:
\begin{table}[H]
\centering
\begin{tabular}{|p{0.5cm}|p{2cm}|p{7cm}|p{2cm}|p{2cm}|} 
\hline \textbf{ID} \textbf{Wer/Was?} & \textbf{Was?} & \textbf{Wann?} & \textbf{An was/wen?} \\\hline
\hline 1 & WLAN-Modul & 1 & wenn er in der Basis ist bzw. bereit ist eine Speise auszuliefern & Server \\\hline
...
\begin{itemize}
    \item \textbf{ID 1}\\
Damit der Server weiß, wann der Hexacopter sich in der Basis befindet und dadurch bereit ist eine Speise auszuliefern, schickt der Hexacopter ihm einen Einser.
    \item \textbf{ID 2}\\
...
"<<...Farbcode'GS'...Farbcode'GS'---Farbcode>>" \\
...
Der Farbcode wird als Binärzahl mitgeschickt. Das Kürzel "GS" steht für "Group seperator" und hilft dem Modul die unterschiedlichen Farbcodes zu unterteilen.
...
Dieser Vorgang wird beispielsweise folgendermaßen bewerkstelligt:
\lstset{language = java}
  	\begin{lstlisting}
String querytischid = "SELECT t.id  FROM tischdaten t JOIN bestellungen b ON (b.fk_tischnr = t.id) WHERE ausliefern = 1 && serviert = 0";
Statement stmttischid = connection.createStatement();
ResultSet rstischid = stmttischid.executeQuery(querytischid);
	\end{lstlisting}
  \end{itemize}  
\\ \\
