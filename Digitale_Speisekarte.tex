...
Um die dadurch entstandene Sass-Datei in den Twig beziehungsweise HTML-Dateien einbinden zu können wurde sie mithilfe des Task-Runners Gulp kompiliert.\\ \\
...
Der {Apache HTTP Server\cite{apachehttpserver}} ist eine freie Software, welche von der Apache Software Foundation entwickelt wurde. Der Apache Webserver kann mit den Betriebssystemen Linux, Unix, Windows, Mac OS X, Betware und OpenBSD arbeiten. Dieser Server ermöglicht es Webseiten als http-Services bereitzustellen, damit sie anschließend in Web-Browsern aufgerufen werden können.
...
Auf der {Webseite von Smyfony\cite{symfony}} wird eine große Dokumentation angeboten, welche Einsteigern dabei hilft sich damit zurechtzufinden. Um mit Symfony arbeiten zu können wurde zuerst der Symfony-Installer integriert. Bei einem Gerät mit dem Betriebssystem Mac OS X wurde dies durch die Eingabe folgender Befehle in das Terminal bewerkstelligt:
...
{DOM\cite{dom}} (Document Object Model) ist ein vom W3-Konsortium standardisiertes Schema für Programmiersprachen, welches beschreibt, wie auf Elemente von XML- und HTML-Dateien zugegriffen werden kann. Solche Auszeichnungssprachen können mit einer hierarchischen Baumstruktur abgebildet werden. Die einzelnen Teile dieses Baumes werden als Knoten beziehungsweise als "nodes" bezeichnet.  Es gilt hierbei in Element-, Attribut- und Textknoten zu unterscheiden. DOM ermöglicht es auf diese Knoten zuzugreifen und ihre Inhalte zu verändern, was sehr praktisch für Webapplikationen sein kann. Diese Funktionalität wurde beispielsweise für das Feld, welches den Verbindungsstatus zwischen Server und Hexacopter anzeigt, auf dem Hauptscreen des Admininterfaces, verwendet.
...
Für den Aufbau der Screens wurden drei verschiedene Grundkonzepte angewendet:
\begin{itemize}
...
Der Hauptscreen enthält ebenfalls einen Header, mit nahezu demselben Schema wie bei der Speisekarte. Der einzige Unterschied liegt darin, dass sich die Navigationsbuttons bei diesem Screen links und rechts vom Titel in Form von Symbolen anstatt von Textelementen befinden. Die Bestellungen werden als Liste dargestellt. Unter dieser Liste wird der Verbindungsstatus vom Hexacopter zum Server in einer Alert-Box angezeigt.
  \end{itemize}
Das Design der Screens ist dem des Blogs sehr ähnlich, da die Layouts aneinander angepasst wurden. Der Hintergrund besteht daher ebenfalls aus einem Tisch, der den Esstisch symbolisieren soll. Die Inhalte werden jeweils in leicht durchsichtigen weißen div-Boxen angezeigt. Dieses Layout ist absichtlich nicht sehr aufwendig gestaltet, damit es elegant und modern auf den Anwender wirkt.
...
\textbf{Screens der digitalen Speisekarte}
			\begin{figure}[H]
			\begin{centering}
			\fbox{ \includegraphics[width = 0.45\textwidth]{Bilder/Jok_login} }
			\fbox{ \includegraphics[width = 0.45\textwidth]{Bilder/Jok_tnr} }
			\par\end{centering}
			...
			\begin{figure}[H]
			\begin{centering}
			\fbox{ \includegraphics[width = 0.45\textwidth]{Bilder/Jok_speisekarte} }
			\fbox{ \includegraphics[width = 0.45\textwidth]{Bilder/Jok_status} }
			\par\end{centering}
...
\pagebreak
\section{Testphase}
Für die Testphase wurden Testfälle erstellt, welche abgesehen von TC-4 von externen Personen getestet wurden.
